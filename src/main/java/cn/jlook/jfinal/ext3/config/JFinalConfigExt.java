 ...
 */
package cn.jlook.jfinal.ext3.config;
...
public abstract class JFinalConfigExt extends JFinalConfig {
     ...
     * AutoBindRoutes自动取xxController对应的class的Controller之前的xx作为controllerKey(
     * path)<br/>
     * 如：MyUserController => myuser; UserController => user; UseradminController
     * => useradmin<br/>
     */
    public void configRoute(Routes me) {
        me.add(new AutoBindRoutesExt().setDefaultViewPath(getProperty("app.default.view.path", "")).autoScan(false));
        // config others
        configMoreRoutes(me);
    }
    ...
    private DruidEncryptPlugin getDruidPlugin(String ds) {
        ...
        DruidEncryptPlugin dp = new DruidEncryptPlugin(url, this.getProperty(String.format(USER_TEMPLATE, ds)),
        ...
        if (!this.geRuned) {
            ...
            ModelGeneratorExt modelGe = new ModelGeneratorExt(this.getModelPackage(), this.getBaseModelPackage(),
                    this.getModelOutDir());
            Generator ge = new Generator(dp.getDataSource(), baseGe, modelGe);
            ge.setGenerateDataDictionary(this.getGeDictionary());
            ge.setRemovedTableNamePrefixes(getRemovedTableNamePrefixes());
            ge.generate();
        }
    ...
    private String[] getRemovedTableNamePrefixes() {
        loadPropertyFile();
        String prefixes = getProperty("ge.prefixes", "");
